...
@Component({
  ...
  template: `
    <!-- ✅ FIX 1: Wrapped in page-layout with navbar (same pattern as other pages) -->
...
export class InboxComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // ✅ FIX 2: Handle queryParams — use 'name' to populate recipientName in fake conversation
  ...
  // ✅ FIX 3: Accept contactName param and use it in the fake conversation
  openOrCreateConversation(contactEmail: string, contactName?: string): void {
    ...
    if (existing) {
      ...
      // Create a temporary conversation object so the user can send a first message
      const fakeConv: Conversation = {
        ...
        senderViewed: true,
        partnerViewed: false,
        // ✅ FIX 4: Populate recipientName so the chat header shows the real name
  ...
  sendMessage(): void {
    ...
    this.messagerieService.sendMessage(otherEmail, this.newMessage).subscribe({
      next: (msg) => {
        this.messages.push(msg);
        this.newMessage = '';
        this.sending = false;

        // If this was a fake (new) conversation, reload real conversations
