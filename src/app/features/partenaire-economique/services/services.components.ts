...
@Component({
  ...
  styles: [`
    ...
    .clear-search-btn {
      padding: 0.6rem 1.4rem; background: linear-gradient(135deg, #7c3aed, #a855f7);
  ...
  onSearch(): void {
    ...
    this.filtered = this.services.filter(s => {
      return [
        ...
        s.collaborationType, s.activityDomain,
        s.expectedBenefits, s.collaborationDuration,
        s.contactPerson, s.address,
        s.availability,
        s.region?.name,
